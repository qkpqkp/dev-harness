...
import { getBranch, getRecentCommits, getStatus } from "../core/git.js";
import { logInfo } from "../core/logger.js";
import { readPackageInfo, summarizeTree, taskKeywords } from "../core/project.js";
import { requireLatestRun, harnessPath } from "../core/runs.js";
import { runCommand, truncateOutput } from "../core/shell.js";

async function findRelatedFiles(cwd: string, task: string): Promise<string[]> {
  const keywords = taskKeywords(task);
  if (keywords.length === 0) return [];

  const fileQuery = keywords.map((word) => `-g "*${word}*"`).join(" ");
  const nameResult = await runCommand(`rg --files ${fileQuery}`, cwd, 20_000);
  const byName = nameResult.exitCode === 0 ? nameResult.stdout.split(/\r?\n/).filter(Boolean) : [];

  const contentQuery = keywords.slice(0, 6).join("|");
  const contentResult = await runCommand(`rg -l -i "${contentQuery}" --glob "!node_modules/**" --glob "!dist/**" --glob "!.git/**"`, cwd, 20_000);
  const byContent = contentResult.exitCode === 0 ? contentResult.stdout.split(/\r?\n/).filter(Boolean) : [];
...
export async function commandContext(args: string[]): Promise<void> {
  ...
  const task = args.join(" ").trim() || (await readTextIfExists(path.join(run.dir, "task.md"))).replace(/^# Task\s*/i, "").trim();
  const packageInfo = await readPackageInfo(cwd);
  const relatedFiles = await findRelatedFiles(cwd, task);

  const harnessProject = await readTextIfExists(harnessPath(cwd, "PROJECT.md"));
  const harnessChecks = await readTextIfExists(harnessPath(cwd, "CHECKS.md"));
  const harnessDecisions = await readTextIfExists(harnessPath(cwd, "DECISIONS.md"));

  const scriptLines = Object.entries(packageInfo.scripts).map(([name, script]) => `- ${name}: \`${script}\``);
...
`;
  ...
  logInfo(`updated: ${path.relative(cwd, path.join(run.dir, "context.md"))}`);
}
