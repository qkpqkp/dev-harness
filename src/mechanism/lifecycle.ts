...
export const RUN_STATUSES = ["planned", "in_progress", "reviewed", "summarized"] as const;
...
export async function createRun(
  ...
  await setRunStatus(dir, "planned");
...
export async function getLatestRun(
  ...
  const pointer = (
    await readTextIfExists(harnessPath(cwd, "current-run"))
  ).trim();
  if (pointer) {
...
export async function requireLatestRun(
  cwd: string,
): Promise<{ id: string; dir: string }> {
  const run = await getLatestRun(cwd);
  if (!run) {
    throw new Error(
      'No harness run found. Start with: devh plan "task description"',
    );
  }
  return run;
}

export async function getRunStatus(dir: string): Promise<RunStatus> {
  const content = await readTextIfExists(path.join(dir, "status"));
  const trimmed = content.trim();
  if (RUN_STATUSES.includes(trimmed as RunStatus)) {
    return trimmed as RunStatus;
  }
  return "planned";
}
...
export async function setRunStatus(
  ...
  await writeText(path.join(dir, "status"), status);
}
